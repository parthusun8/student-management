...
function Home() {
    ...
    const handleEdit = async () => {
        ...
        await axios
            .put("http://localhost:4000/api/departments/" + prevDepartmentNo + "/", {
    ...
    const handleDelete = async (row) => {
        ...
        await axios
            .delete("http://localhost:4000/api/departments/" + row.dept_no + "/")
            .then((res) => {
                console.log("Delete Successfull");
                setDepartments(
                    departments.filter((department) => department.dept_no !== row.dept_no)
                );
            })
            .catch((err) => {
                console.log(err);
    ...
    const handleAdd = async () => {
        // Handle add action
        if (
            departments.some((department) => department.dept_no === newDepartmentNo)
        ) {
            ...
            await axios
                ...
                .then((res) => {
                    console.log(res);
                    setDepartments(
                        [...departments, res.data].sort((a, b) => a.dept_no - b.dept_no)
                    );
                    closeModal();
                })
                .catch((err) => {
                    ...
                    <TableBody>
                        {departments.map((row) => (
                            <TableRow key={row.dept_no}>
                                ...
                                <TableCell align="center">
                                    <Link
                                        to={`/subjects?id=${row.dept_no}&dept=${row.dept_name}`}
                                    >
                                        <Button variant="outlined">Subjects</Button>
                                    </Link>
                                    <Link
                                        to={`/departments?id=${row.dept_no}&dept=${row.dept_name}`}
                                    >
                                        <Button variant="outlined">Sections</Button>
                                    </Link>
            ...
            >
                <FormControl>
                    <TextField
                        ...
                        onChange={(e) =>
                            setNewDepartmentNo(
                                parseInt(e.target.value != NaN ? e.target.value : 0)
                            )
                        }
                    />
                    <TextField
            ...
            >
                <FormControl>
                    <TextField
                        ...
                        label="Department No"
                        variant="outlined"
                        value={newDepartmentNo}
                        onChange={(e) => setNewDepartmentNo(parseInt(e.target.value))}
                    />
                    <TextField
                        id="dept_name"
                        name="dept_name"
                    ...
                    <Button variant="contained" onClick={editModalClose}>
                        Cancel
                    </Button>
                </FormControl>
            </Modal>
